 ...
 */
public class TypeAuthTemplate {

    //索引库名
    static String index = "my_index";
    //类型名称
    static String type = "typeAuthResource";
    ...
    public static void builkIndex(Map<String, Object> map){
        ...
        //如果是学科目录表变动
        if(map.get("typeId") != null && StringUtils.isNotBlank(map.get("typeId").toString())){
            boolQueryBuilder.should(QueryBuilders.termQuery("typeId", map.get("typeId").toString()));
            boolQueryBuilder.should(QueryBuilders.termQuery("arResourceId", map.get("typeId").toString()));
            id = id.append(map.get("typeId").toString()).append(ParaseUtil.SEP).append("null") ;
        }
        //如果是资源表变动
        if(map.get("arId") != null && StringUtils.isNotBlank(map.get("arId").toString())){
            ...
            //如果资源类型是学科目录（resourceType--> 3）
            if(map.get("arResourceType") != null && ParaseUtil.AR_RESOURCE_TYPE_TYPE.equals(map.get("arResourceType"))){
                if(map.get("arResourceId") != null && StringUtils.isNotBlank(map.get("arResourceId").toString())){
                    boolQueryBuilder.should(QueryBuilders.termQuery("typeId", map.get("arResourceId").toString()));
                }
        ...
        if(resultHits.getTotalHits() > 0){
            for (SearchHit hit : resultHits) {
                ...
                id = id.append(hitMap.get("typeId")).append(ParaseUtil.SEP).append(hitMap.get("arId"));
                hitMap.put("id", id);
                //将现有资源更新
